(function () {
  ...
  function renderMonth() {
    ...
    var atMin = new Date(y, m, 1) <= thisMonth;
    el('bkPrevMonth').disabled = atMin;
    el('bkPrevMonth').style.opacity = atMin ? '0.3' : '1';
    el('bkPrevMonth').style.cursor = atMin ? 'not-allowed' : 'pointer';
    ...
    for (var day = 1; day <= lastDay; day++) {
      ...
      btn.style.cssText = 'width:100%;padding:10px 0;border-radius:8px;font-size:14px;font-weight:500;font-family:inherit;transition:all .15s;box-sizing:border-box;' +
        (isPast
      ...
      if (!isPast) {
        btn.addEventListener('mouseenter', function () { if (state.date !== this.dataset.date) this.style.background = 'rgba(255,255,255,.13)'; });
        btn.addEventListener('mouseleave', function () { if (state.date !== this.dataset.date) this.style.background = 'rgba(255,255,255,.07)'; });
        btn.addEventListener('click', function () {
          state.date = this.dataset.date;
          state.time = null;
          el('bkDetailsCard').style.display = 'none';
  ...
  function loadSlots(date) {
    ...
    fetch(WORKER + '/api/availability?date=' + date)
      ...
      .then(function (d) {
        state.slots = d.slots || [];
        if (state.slots.length > 0) renderSlots();
        else grid.innerHTML = '<div style="grid-column:1/-1;text-align:center;padding:24px;color:rgba(255,255,255,.4);font-size:15px">No available slots for this date</div>';
      })
      .catch(function () {
  ...
  function renderSlots() {
    ...
    state.slots.forEach(function (slot) {
      ...
      btn.style.cssText = 'padding:12px 8px;border-radius:8px;font-size:14px;font-weight:500;font-family:inherit;cursor:pointer;transition:all .15s;box-sizing:border-box;' +
      ...
      btn.addEventListener('mouseenter', function () { if (state.time !== this.dataset.time) this.style.background = 'rgba(255,255,255,.13)'; });
      btn.addEventListener('mouseleave', function () { if (state.time !== this.dataset.time) this.style.background = 'rgba(255,255,255,.07)'; });
      btn.addEventListener('click', function () {
        state.time = this.dataset.time;
        renderSlots();
        el('bkSlotChip').textContent = fmtDateFull(state.date).split(',')[0] + ', ' + fmtTime(state.time);
        var details = el('bkDetailsCard');
  ...
  el('bkBookBtn').addEventListener('click', function () {
    ...
    fetch(WORKER + '/api/book', {
      ...
      body: JSON.stringify({
        name: name, email: email, company: 'N/A',
        description: 'Discovery call booking',
        start: state.time, end: endTime,
        notes: '', locale: 'en', source: 'direct'
      })
    })
      .then(function (r) { return r.json(); })
      .then(function (d) {
        if (d.success) showConfirmation(name);
        else {
